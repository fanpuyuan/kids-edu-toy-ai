...
- **双引擎多模态入库 (新增)**：
    - RAG 服务不再锁死本地计算。根据网关传入的 `embedding_provider`，它能动态决定使用本地的 HuggingFace `BAAI/bge-large-zh` 中文语义模型，或是调用云端（如阿里云 DashScope Text Embedding）构建张量，压入 ChromaDB 轻量数据库中，并将文档打上 Provider 隔离标签。
    - **字词入库**：同步开启倒排词树 BM25(Jieba 分词)。因为在“找孙悟空第十八页讲了什么”这类问题中，大语言模型生成的纯向量距离判断经常对于特殊名词敏感度欠缺，这里使用双路备份保障命中。
